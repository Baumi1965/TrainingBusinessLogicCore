...
namespace Training.BusinessLogic.Preis
{
    public class PreisLokal
    {
        public static async Task<SyncResult> DoSyncAsync(string aktion, string daten)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Aktion: {aktion}, Tabelle: preise");
                Console.WriteLine($"ID: {jsonObject.ID}");
                Console.WriteLine($"Guid: {jsonObject.Guid}");

                var entityType = System.Type.GetType("Training.BusinessLogic.Preis.ModelsLokal.preislokal");

                if (entityType == null)
                {
                    Console.WriteLine("Entity type not found.");
                    result.Error = true;
                    result.ErrorMessage = "Preis Type nicht gefunden.";
                    return result;
                }
