    ...
    public class KundenLokal
    {
        public static async Task<int> CountAsync()
        {
            try
            {
                ...
                return await Uow._uowLokal.Query<kundenlokal>().CountAsync();
        ...
        public static async Task<SyncResult> DoSyncAsync(string aktion, string daten)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Aktion: {aktion}, Tabelle: kunden");
                Console.WriteLine($"ID: {jsonObject.ID}");
                Console.WriteLine($"Guid: {jsonObject.Guid}");

                var entityType = System.Type.GetType("Training.BusinessLogic.Kunden.ModelsLokal.kundenlokal");

                if (entityType == null)
                {
                    Console.WriteLine("Entity type not found.");
                    result.Error = true;
                    result.ErrorMessage = "Kunden Type nicht gefunden.";
                    return result;
                }
