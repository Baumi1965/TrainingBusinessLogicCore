...
{
    [Persistent("kunden")]
    public class kundenlokal : XPLiteObject
    {
        public kundenlokal(Session session) : base(session) { }
		...
		protected override void OnSaving()
		{
			if (!IsDeleted && ID == 0)
			{
				...
				var maxId = Session.Evaluate<kundenlokal>(CriteriaOperator.Parse("Max(ID)"), null);
				ID = (maxId as int? ?? 0) + 1;
			}
