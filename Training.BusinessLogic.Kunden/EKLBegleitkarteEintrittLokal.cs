...
{
    public class EKLBegleitkarteEintrittLokal
    {
        public static async Task<SyncResult> DoSyncAsync(string aktion, string daten)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Aktion: {aktion}, Tabelle: eklbegleitkarteeintritt");
                Console.WriteLine($"ID: {jsonObject.ID}");
                Console.WriteLine($"Guid: {jsonObject.Guid}");

                var entityType = System.Type.GetType("Training.BusinessLogic.Kunden.ModelsLokal.eklbegleitkarteeintrittlokal");

                if (entityType == null)
                {
                    Console.WriteLine("Entity type not found.");
                    result.Error = true;
                    result.ErrorMessage = "EKLBegleitkarteEintritt Type nicht gefunden.";
                    return result;
                }
