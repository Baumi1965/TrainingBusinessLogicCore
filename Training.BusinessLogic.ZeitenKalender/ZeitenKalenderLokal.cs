...
{
    public class ZeitenKalenderLokal
    {
        public static async Task<SyncResult> DoSyncAsync(string aktion, string daten)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Aktion: {aktion}, Tabelle: zeitenkalender");
                Console.WriteLine($"ID: {jsonObject.ID}");
                Console.WriteLine($"Guid: {jsonObject.Guid}");

                var entityType = System.Type.GetType("Training.BusinessLogic.ZeitenKalender.ModelsLokal.zeitenkalenderlokal");

                if (entityType == null)
                {
                    Console.WriteLine("Entity type not found.");
                    result.Error = true;
                    result.ErrorMessage = "ZeitenKalender Type nicht gefunden.";
                    return result;
                }
