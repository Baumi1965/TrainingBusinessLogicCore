...
namespace Training.BusinessLogic.ZeitenKalender.ModelsLokal
{
    [Persistent("zeitenkalender")]
    public class zeitenkalenderlokal : XPLiteObject
    {
        public zeitenkalenderlokal(Session session) : base(session) { }
        ...
        protected override void OnSaving()
        {
	        if (!IsDeleted && ID == 0)
	        {
		        ...
		        var maxId = Session.Evaluate<zeitenkalenderlokal>(CriteriaOperator.Parse("Max(ID)"), null);
		        ID = (maxId as int? ?? 0) + 1;
	        }
